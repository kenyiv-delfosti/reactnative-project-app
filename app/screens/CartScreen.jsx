                ...
                {cartItems.length === 0 || !cartItems ? (
                    ...
                    <ScrollView className="w-full flex-1">
                        <View className="flex space-y-4">
                            <FlatList data={cartItems} keyExtractor={item => item.data._id} renderItem={({ item }) => (<CartItemCart item={item.data} qty={item.qty} />)} />
                        </View>
                        <View className="w-full p-8">
                            <View className="w-full px-2 h-16 rounded-xl bg-white flex-row items-center justify-center">
                                <TextInput placeholder='Promo Code' className="text-base px-4 font-semibold text-[#555] flex-1 py-1" />
                        ...
                        <View className="w-full px-8 my-4">
                            <TouchableOpacity onPress={() => navigation.navigate("CheckoutScreen")} className="w-full py-3 rounded-xl bg-black flex items-center justify-center">
...
export const CartItemCart = ({ item, qty }) => {
            ...
            <View className="flex-row px-6 w-full items-center my-1">
                ...
                <View className="flex items-center space-y-2 ml-3">
                    <View className='flex items-start justify-center'>
                        ...
                        <Text className="text-lg font-semibold text-[#777]">{item?.shortDescription}</Text>
                        {/* <View className="flex-row items-center justify-center space-x-3">
                        <Text>$ {item?.price * qty}</Text>
                        <Text>{qty}</Text>
                    </View> */}
                        <Text className="text-lg font-bold text-black">$ {(item?.price * qty).toFixed(2)}</Text>
